...
def pe_parser_misc(data):
    ...
    try:
        ...
        rows.append([
            "PE Characteristics (raw)", hex(
                data.pe.coff_hdr.characteristics,
            ),
        ])
        ...
        rows.append([
            "Timestamp", datetime.fromtimestamp(
                data.pe.coff_hdr.time_date_stamp, timezone.utc,
            ).strftime("%Y-%m-%d %H:%M:%S"),
        ])
        rows.append([
            "DLL Characteristics (raw)", hex(
                data.pe.optional_hdr.windows.dll_characteristics,
            ),
        ])
        dllcharacts = pe_parser_dll_char_lookup(
            data.pe.optional_hdr.windows.dll_characteristics,
        )
        ...
        if image64 := getattr(data.pe.optional_hdr.windows, "image_base_64", False):
            ...
            rows.append([
                "Base Address", hex(
                    data.pe.optional_hdr.windows.image_base_32,
                ),
            ])
        rows.append(
            [
                "Entrypoint", hex(
                    data.pe.optional_hdr.std.address_of_entry_point,
                ),
            ],
        )
        rows.append([
            "Base of Code", hex(
                data.pe.optional_hdr.std.base_of_code,
            ),
        ])
...
def pe_parser_extract(data, args):
    ...
    for section in data.pe.sections:
        if section.name == args or section.name == f".{args}":
            return section.body.decode(u"UTF-8", errors="ignore")


def pe_parser_certificate(data):
    """Extracts a certificate from a PE file"""
    for cert in data.pe.certificate_table.items:
        return cert.certificate_bytes
...
def pe_parser_datadirs(data):
    ...
    rows = []
    for ddir in [
        'architecture', 'base_relocation_table', 'bound_import', 'certificate_table', 'clr_runtime_header', 'debug', 'delay_import_descriptor',
        'exception_table', 'export_table', 'global_ptr', 'iat', 'import_table', 'load_config_table', 'resource_table', 'tls_table',
    ]:
        rows.append([
            ddir, getattr(data.pe.optional_hdr.data_dirs, ddir).size, hex(
                getattr(data.pe.optional_hdr.data_dirs, ddir).virtual_address,
            ),
        ])
...
def pe_parser_extract_datadir(data, datadir):
    ...
    content = Deject.r2_handler.cmd(
        "p8 {} @ {}".format(
            getattr(data.pe.optional_hdr.data_dirs, datadir).size, hex(
                base + int(
                    getattr(
                        data.pe.optional_hdr.data_dirs,
                        datadir,
                    ).virtual_address,
                ),
            ),
    ...
    f.close()
    return f"SSDEEP Hash: {sshash}" + \
        f"[+] Saved output of {datadir} to {filepath}/{datadir}"
...
def pe_parser_sections(data):
    ...
    for section in data.pe.sections:
        ...
        rows.append([
            "Section Characteristics (raw)",
            hex(section.characteristics),
        ])
        rows.append([
            "Section Characteristics", "\n".join(
                pe_parser_sect_char_lookup(section.characteristics),
            ),
        ])
    t = {"header": ["Key", "Value"], "rows": rows}
    return t
